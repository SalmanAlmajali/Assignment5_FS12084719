...
async function handleClickEditButton(bookId) {
  try {
    ...
    const bookData = await fetch(`http://localhost:3333/books/${bookId}`);
    currentBook = await bookData.json();
...
async function fetchBooks() {
  try {
    ...
    */
    ...
    const buku = await fetch(`http://localhost:3333/books`);
...
async function addBook(book) {
  try {
    ...
    */
    ...
    const addBuku = await fetch(`http://localhost:3333/books`, {
      method: 'POST',
      headers: {
...
async function editBook(book) {
  try {
    ...
    */
    ...
    const bukuSunting = await fetch(`http://localhost:3333/books/${book.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
...
async function deleteBook(bookId) {
  try {
    ...
    */
    ...
    const bukuHapus = await fetch(`http://localhost:3333/books/${bookId}`, {
      method: 'DELETE',
    })
    if(bukuHapus){
      console.log(bukuHapus);
    }
  } catch (error) {
    console.log(error);
    console.log('Terjadi kesalahan saat menghapus buku');
